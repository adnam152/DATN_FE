...
function ProductItem({ product }) {
  return (
  <>
  <Link to='/san-pham/1' className="bg-white me-7 mb-7 rounded-lg p-4 w-60 h-max shadow-sm hover:shadow-lg hover:scale-105 transition-all cursor-pointer">
      ...
      <div className="product-info mt-4">
        <h3 className="product-name text-lg font-semibold text-slate-500">{product.name}</h3>
        <div className="product-rate text-yellow-400 my-2">
          {[...Array(5)].map((_, index) => (
            <i
              key={index}
              className={`fa-solid fa-star ${index < product.rating ? 'text-yellow-400' : 'text-gray-300'
                }`}
            ></i>
          ))}
        </div>
        <div className="product-detail flex justify-between mt-2">
          <div className="product-price text-lg font-bold mt-1">{product.selling_price}</div>
